const reduceMotionUniko = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
let unikoInitialized = false;
...
function initUnikoCarousel() {
  ...
  const animateToIndex = (newIndex) => {
    ...
    gsap.to(track, {
      x: -(currentIndex * step),
      duration: reduceMotionUniko ? 0.01 : 0.7,
      ease: "power3.inOut",
      onComplete: () => {
        isAnimating = false;
      },
...
function initUnikoPage() {
  if (unikoInitialized) {
    return;
  }
  unikoInitialized = true;

  if (reduceMotionUniko) {
    gsap.set(".main-nav .nav-link", { opacity: 1, y: 0, clearProps: "transform" });
    gsap.set(".uniko-main", { autoAlpha: 1 });
    gsap.set([".uniko-title", ".uniko-work"], {
      opacity: 1,
      y: 0,
...
function initUnikoWhenReady() {
  if (document.documentElement.dataset.pageEnterReady === "true") {
    initUnikoPage();
  } else {
    document.addEventListener("page:enter-ready", initUnikoPage, { once: true });
  }
}
...
if (document.documentElement.dataset.componentsReady === "true") {
  ...
  document.addEventListener("components:ready", initUnikoWhenReady, { once: true });
}
