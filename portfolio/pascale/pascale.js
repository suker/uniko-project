const reduceMotionPascale = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
let pascaleInitialized = false;
...
function initPascaleCarousel() {
  ...
  const animateToIndex = (newIndex) => {
    ...
    gsap.to(track, {
      x: -(currentIndex * step),
      duration: reduceMotionPascale ? 0.01 : 0.7,
      ease: "power3.inOut",
      onComplete: () => {
        isAnimating = false;
      },
...
function initPascalePage() {
  if (pascaleInitialized) {
    return;
  }
  pascaleInitialized = true;

  if (reduceMotionPascale) {
    gsap.set(".main-nav .nav-link", { opacity: 1, y: 0, clearProps: "transform" });
    gsap.set(".pascale-main", { autoAlpha: 1 });
    gsap.set([".pascale-title", ".pascale-role", ".pascale-profile"], {
      opacity: 1,
      y: 0,
...
function initPascaleWhenReady() {
  if (document.documentElement.dataset.pageEnterReady === "true") {
    initPascalePage();
  } else {
    document.addEventListener("page:enter-ready", initPascalePage, { once: true });
  }
}

if (document.documentElement.dataset.componentsReady === "true") {
  initPascaleWhenReady();
} else {
  document.addEventListener("components:ready", initPascaleWhenReady, { once: true });
}
